<%@page import="com.techlabs.services.DepartmentService"%>
<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<% int deptId = Integer.parseInt(request.getParameter("deptId").toString()); 
	boolean resultDept = new DepartmentService().deleteDepartmentById(deptId);
%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1">
<title>Department Action!</title>
</head>
<body>
<%
if(resultDept){
	out.println("<h1>Department is successfully deleted!</h1>");
}else{
	out.println("<h1>Oops!Something went wrong!</h1>");
}

%>
<a href="manageDepartment.jsp">Go back to previous page!</a>
</body>
</html>